import { BriefcaseBusiness, ChartNoAxesCombined, ChevronRight, LayoutDashboard, LogOut, Menu, MessageCircle, Newspaper, Settings, X } from 'lucide-react';
...
const nav = [
  { to: '/app', label: 'Visão geral', helper: 'Resumo simples da sua carteira', icon: LayoutDashboard, end: true },
  { to: '/app/dashboard-tecnico', label: 'Painel técnico', helper: 'Análises e gráficos detalhados', icon: ChartNoAxesCombined },
  { to: '/app/noticias', label: 'Notícias', helper: 'Contexto para seus investimentos', icon: Newspaper },
  { to: '/app/chat', label: 'Chat', helper: 'Dúvidas em linguagem simples', icon: MessageCircle },
  { to: '/app/carteiras', label: 'Carteiras', helper: 'Monte, organize e acompanhe', icon: BriefcaseBusiness },
  { to: '/app/configuracoes', label: 'Configurações', helper: 'Conta e preferências', icon: Settings },
];
...
export function AppShell() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-app)] text-[var(--text-main)]">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[18rem] flex-col border-r border-[var(--border-soft)] bg-[var(--bg-surface-strong)] p-5 shadow-[var(--shadow-float)] transition-transform duration-200 lg:translate-x-0 lg:shadow-none ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="ai-surface mt-7 rounded-2xl p-4">
          <p className="eyebrow">Seu espaço financeiro</p>
          <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">Acompanhe, entenda e aprofunde no seu ritmo.</p>
        </div>

        <nav className="mt-6 flex-1 space-y-1 overflow-y-auto" aria-label="Ferramentas Operum">
          {nav.map(({ to, label, helper, icon: Icon, end }) => (
            <NavLink
              key={to}
              to={to}
              end={end}
              onClick={() => setOpen(false)}
              className={({ isActive }) => `group flex items-start gap-3 rounded-xl px-3 py-3 text-sm transition ${isActive ? 'bg-[var(--complementary-soft)] text-[var(--text-main)]' : 'text-[var(--text-muted)] hover:bg-[var(--bg-surface-muted)] hover:text-[var(--text-main)]'}`}
            >
              {({ isActive }) => (
                <>
                  <span className={`mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg ${isActive ? 'bg-[#684cf2] text-white' : 'bg-[var(--bg-surface-muted)] text-[var(--brand)]'}`}><Icon size={17} /></span>
                  <span className="min-w-0 flex-1"><span className="block font-semibold">{label}</span><span className="mt-0.5 block text-[11px] leading-4 opacity-75">{helper}</span></span>
                  <ChevronRight size={15} className="mt-2 shrink-0 opacity-40 transition group-hover:translate-x-0.5" />
                </>
              )}
            </NavLink>
          ))}
        ...
        <div className="mt-4 border-t border-[var(--border-soft)] pt-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{user?.name ?? 'Conta'}</p><p className="truncate text-xs text-[var(--text-muted)]">{user?.email}</p></div>
      ...
      <main className="min-w-0 lg:ml-[18rem]">
